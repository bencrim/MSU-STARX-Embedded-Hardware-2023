...
This program gives a good look into how we use the Adafruit_MPU6050.h and Adafruit_Sensor.h libraries. 
The first thing we do is create two objects of type Adafruit_MPU6050 that will be used to access the data coming from each gyro.
...
which is a sensors_vec_t object assigned to our gyros. This is why later in Tilt_sensor_gyro.ino, we call the data using a1.gyro.x. The x value from the gyro is a value
in an object in an object. (This took me a very long time to finally understand)

The next major part of this program is in the void tilt_setup() function. This function attempts to initialize each gyro using the .begin function with 
...
This process is done for both gyros and then the program continues on to the void loop(). 
